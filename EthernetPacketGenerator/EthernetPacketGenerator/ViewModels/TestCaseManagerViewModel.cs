...
public class TestCaseManagerViewModel : ViewModelBase
{
    ...
    {
        var path = FindBestTestCaseFile();
        if (path == null) return;
        try { ApplyLoadedGroups(TestCaseSerializer.LoadFromFile(path)); }
        catch { }
    }
    ...
    {
        const string mac = "C8:4D:44:25:2D:37";
    ...
    private SequenceItem? ConvertRowToSequenceItem(TestScenarioStep row)
    {
        ...
        if (action.Equals("FdbWrite", StringComparison.OrdinalIgnoreCase))
            return new SequenceItem(new SequenceEvent { EventType = SequenceEventType.FdbWrite, MacAddress = NormalizeMac(row.Value), Port = ParsePort(row.Expected) });
